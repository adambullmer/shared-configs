...
function resolveSchemaPath() {
  try {
    ...
    return `./${relativePath.replaceAll(String.raw`\\`, "/")}`;
  } catch {
...
function parseJsonc(content) {
  ...
  let cleaned = content.replaceAll(/\/\/.*$/gm, "");
...
function formatConfig(config) {
  ...
  const lines = ["{", `  "$schema": "${schemaPath}",`, "  // Configuration managed by @adambullmer/oxfmt-config"];
...
try {
  const sourceContent = readFileSync(sourceConfigPath, "utf8");
  const sourceConfig = parseJsonc(sourceContent);

  if (existsSync(targetConfigPath)) {
    const existingContent = readFileSync(targetConfigPath, "utf8");
    ...
    writeFileSync(targetConfigPath, formattedConfig, "utf8");
